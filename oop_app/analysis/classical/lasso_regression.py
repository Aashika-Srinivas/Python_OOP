...
class LassoRegression(AnalysisInterface):
    ...
    def build_model(self, tune_alpha=False, lasso_alpha=None):
        try:
            ...
            if tune_alpha:
                ...
                self.lasso_reg = Lasso(alpha=tuned_alpha, random_state=42)
            else:
                if lasso_alpha is None:
                    self.lasso_reg = Lasso(random_state=42)
                else:
                    self.lasso_reg = Lasso(alpha=self.lasso_alpha, random_state=42)

            # fitting the data on the model
            self.lasso_reg.fit(self.X_train, self.y_train)
        except:
            return Response.failure('Error building model')
        return Response.success(self.lasso_reg)
    ...
    def predict_y_from_x_test(self):
        y_predict = self.lasso_reg.predict(self.X_test)
        return y_predict

    def predict_y(self, X_Data):
        y_predict = self.lasso_reg.predict(X_Data)
        return y_predict
